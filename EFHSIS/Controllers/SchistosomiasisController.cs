...
namespace EFHSIS.Controllers
{
    ...
    {
        private readonly ILogger<SchistosomiasisController> _logger;
        private readonly ApplicationDbContext _context;

        public SchistosomiasisController(ILogger<SchistosomiasisController> logger, ApplicationDbContext context)
        {
            _logger = logger;
            _context = context;
        }
        [HttpGet]
        public IActionResult SchistosomiasisHome()
        {
            int year = DateTime.Now.Year;
            DateTime firstDay = new DateTime(year, 1, 1);
            DateTime lastDay = new DateTime(year, 12, 31);

            GraphFunc("SchistosomiasisGraph", Convert.ToDateTime(firstDay), Convert.ToDateTime(lastDay));
            return View();
        }
        [HttpPost]
        public IActionResult SchistosomiasisHome(string filter)
        {
        ...
        public void GraphFunc(string stored_func, DateTime firstDay, DateTime lastDay)
        {
            ...
            var tuberculosis = _context.Schistosomiasis.FromSqlRaw($"EXEC EFHSIS.dbo.{stored_func} @date_start = N'{firstDay}',@date_end = N'{lastDay}',@prov_code = N'{province}';");
